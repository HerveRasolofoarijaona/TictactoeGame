package tictactoe.SimpleJoueur;
...
public class tictactoeOne {
	...
	public void computegame() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					tictactoeOne window = new tictactoeOne();
	...
	public void checkWinner() {
		...
		for(int i = 0; i < 8; i++) {
			if( (board[winPos[i][0]] == "X") && (board[winPos[i][0]] == board[winPos[i][1]]) && (board[winPos[i][1]] == board[winPos[i][2]] )) {
				...
				JOptionPane.showMessageDialog(frame, "X a gagné!");
				chk = 1;
				for(int j = 0; j < 9; j++) {
					used[j] = true;
				}
				...
				JOptionPane.showMessageDialog(frame, "0 a gagné!");
				chk = 1;
				for(int j = 0; j < 9; j++) {
					used[j] = true;
				}
		...
		if(count == 8 && chk == 0){
				...
				JOptionPane.showMessageDialog(frame, "C'est un tirage au sort !");
				chk = 1;
	...
	private void initialize() {
		...
		btnExit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(JOptionPane.showConfirmDialog(frame, "Êtes-vous sûr de vouloir quitter?", "Tic Tac Toe", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_NO_OPTION) {
					System.exit(0);
				}
			}
		});
